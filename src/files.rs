...
pub fn write_file(ignored_content: &str, f: &PathBuf) {
    match File::create(f) {
        Err(why) => panic!("Failed to write file: {}", why),
        Ok(mut file) => file.write_all(ignored_content.as_bytes()).unwrap()
    }
}
...
public with sharing class Test {
    public String gimmeString() {
        ...
        for f in ["file1.cls", "file2.cls", "file3.cls"] {
            write_file(ignored_content, &test_dir.join(f));
        }
        for f in ["file4.cls", "file5.cls"] {
            write_file(found_content, &test_dir.join(f));
        }
